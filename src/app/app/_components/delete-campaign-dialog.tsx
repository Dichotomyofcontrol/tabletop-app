...
export default function DeleteCampaignDialog({ campaignId, campaignName }: {
  ...
  function handleDelete() {
    ...
    fd.set('confirm_name', confirmText);
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center p-6 bg-black/70 backdrop-blur-sm">
      <div className="card-mystic rounded-xl p-6 max-w-md w-full" style={{borderColor: 'rgba(220, 38, 38, 0.4)'}}>
        <h3 className="text-xl font-semibold text-red-300">Delete this campaign?</h3>
        <p className="mt-3 text-sm text-zinc-300 leading-relaxed">
          This permanently deletes <span className="font-semibold text-zinc-50">{campaignName}</span>,
          along with all sessions, characters, and pending invites. This can&apos;t be undone.
        </p>
        <p className="mt-4 text-sm text-zinc-400">
          Type <span className="font-mono text-amber-300">{campaignName}</span> to confirm.
        </p>
